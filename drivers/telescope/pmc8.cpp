...
#define PMC8_VERSION_MAJOR 1
#define PMC8_VERSION_MINOR 10
...
PMC8::PMC8()
{
    ...
    LOGF_INFO("Declination====> = %d.", currentDEC);
...
bool PMC8::initProperties()
{
    ...
    // Adding debug control to the driver
    addDebugControl();
...
void PMC8::getStartupData()
{
    ...
    // must also keep "low level" aware of position to convert motor counts to RA/
    ...
    LOG_INFO("Check the declination for southern hemisphere.");
    LOGF_INFO("Declination====> = %f", currentDEC);
    LOGF_INFO("Latitude====> = %f", latitude);


#if 0
    // FIXEME - Need to handle southern hemisphere for ?
